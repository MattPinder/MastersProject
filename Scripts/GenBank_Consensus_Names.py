...
parser = argparse.ArgumentParser(description='Creates a consensus between two GenBank files. If exactly one entry is hypothetical, it will be overwritten by \
                                             the other. Additionally, if exactly one entry has an associated gene name, it will overwrite the other. If there \
                                             is a conflict, the record from the primary input file will be used.')
...
for locus1 in range(1,allFeatures1):
	if not str(Record1.features[locus1].location) in File2Locations:
	...
	elif "gene" in Record1.features[locus1].qualifiers:
		WriteRecord(Record1,locus1)
        	Entries_from_GenBank1 += 1
		...
		for locus2 in range(1,allFeatures2):
			if str(Record1.features[locus1].location) == str(Record2.features[locus2].location) \
			   and "gene" in Record2.features[locus2].qualifiers:
				WriteRecord(Record2,locus2)
				Entries_from_GenBank2 += 1
			elif str(Record1.features[locus1].location) == str(Record2.features[locus2].location) \
			   and not "gene" in Record2.features[locus2].qualifiers:
				WriteRecord(Record1,locus1)
		                Entries_from_GenBank1 += 1
	elif Record1.features[locus1].qualifiers['product'] == ['hypothetical protein']:
		for locus2 in range(1,allFeatures2):
			if str(Record1.features[locus1].location) == str(Record2.features[locus2].location) \
