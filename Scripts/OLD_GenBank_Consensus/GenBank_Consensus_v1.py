...
#GenBank Consensus Generator, version 1
...
for locus in range(1,allFeatures):
	...
	if "-" in str(Record1.features[locus].location):
        ...
        if not Record1.features[locus].qualifiers['product'] == ['hypothetical protein']:
                if "gene" in Record1.features[locus].qualifiers:
                        Consensus.write("                     /gene=\"" + re.sub("\['|'\]","",str(Record1.features[locus].qualifiers['gene'])) + "\"\n")
		Consensus.write("                     /locus_tag=\"" + re.sub("\['|'\]","",str(Record1.features[locus].qualifiers['locus_tag'])) + "\"\n")
		if "EC_number" in Record1.features[locus].qualifiers:
                        Consensus.write("                     /EC_number=\"" + re.sub("\['|'\]","",str(Record1.features[locus].qualifiers['EC_number'])) + "\"\n")
                if "inference" in Record1.features[locus].qualifiers:
                        for inferences in Record1.features[locus].qualifiers['inference']:
                                Consensus.write("                     /inference=\"" + re.sub("\['|'\]","",inferences) + "\"\n")
                if "note" in Record1.features[locus].qualifiers:
                        for notes in Record1.features[locus].qualifiers['note']:
                                Consensus.write("                     /note=\"" + re.sub("\['|'\]","",notes) + "\"\n")
		Consensus.write("                     /note=\"" + "From " + GenBank1 + "\"\n")
		if Record1.features[locus].type == "CDS":
			Consensus.write("                     /codon_start=\"" + re.sub("\['|'\]","",str(Record1.features[locus].qualifiers['codon_start'])) + "\"\n")
			Consensus.write("                     /transl_table=\"" + re.sub("\['|'\]","",str(Record1.features[locus].qualifiers['transl_table'])) + "\"\n")
		if Record1.features[locus].type == "CDS":
			Consensus.write("                     /product=\"" + re.sub("\['|'\]","",str(Record1.features[locus].qualifiers['product'])) + "\"\n")
			Consensus.write("                     /translation=\"" + re.sub("\['|'\]","",str(Record1.features[locus].qualifiers['translation'])) + "\"\n")
		Entries_from_GenBank1 += 1

        elif not Record2.features[locus].qualifiers['product'] == ['hypothetical protein']:
		if "gene" in Record2.features[locus].qualifiers:
                        Consensus.write("                     /gene=\"" + re.sub("\['|'\]","",str(Record2.features[locus].qualifiers['gene'])) + "\"\n")
		Consensus.write("                     /locus_tag=\"" + re.sub("\['|'\]","",str(Record2.features[locus].qualifiers['locus_tag'])) + "\"\n")
		if "EC_number" in Record2.features[locus].qualifiers:
                        Consensus.write("                     /EC_number=\"" + re.sub("\['|'\]","",str(Record2.features[locus].qualifiers['EC_number'])) + "\"\n")
                if "inference" in Record2.features[locus].qualifiers:
                        for inferences in Record2.features[locus].qualifiers['inference']:
                                Consensus.write("                     /inference=\"" + re.sub("\['|'\]","",inferences) + "\"\n")
                if "note" in Record2.features[locus].qualifiers:
                        for notes in Record2.features[locus].qualifiers['note']:
                                Consensus.write("                     /note=\"" + re.sub("\['|'\]","",notes) + "\"\n")
		Consensus.write("                     /note=\"" + "From " + GenBank2 + "\"\n")
		if Record2.features[locus].type == "CDS":
                	Consensus.write("                     /codon_start=\"" + re.sub("\['|'\]","",str(Record2.features[locus].qualifiers['codon_start'])) + "\"\n")
                	Consensus.write("                     /transl_table=\"" + re.sub("\['|'\]","",str(Record2.features[locus].qualifiers['transl_table'])) + "\"\n")
		if Record2.features[locus].type == "CDS":
			Consensus.write("                     /product=\"" + re.sub("\['|'\]","",str(Record2.features[locus].qualifiers['product'])) + "\"\n")
                	Consensus.write("                     /translation=\"" + re.sub("\['|'\]","",str(Record2.features[locus].qualifiers['translation'])) + "\"\n")
